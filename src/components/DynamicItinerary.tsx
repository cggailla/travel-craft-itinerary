...
export function DynamicItinerary({
  ...
  const regenerateStep = async (stepIndex: number) => {
    ...
    if (!step) return;
    setStepContents(prev => prev.map((content, i) => i === stepIndex ? {
      ...content,
      success: false,
      html: '',
      error: undefined
    ...
    try {
      const {
        generateStepPageHTML
      } = await import('@/services/stepContentService');
      const result = await generateStepPageHTML(tripId, step.step_id);
      setStepContents(prev => prev.map((content, i) => i === stepIndex ? result : content));
    } catch (error) {
  ...
  const renderStaticStep = (step: any, stepIndex: number) => {
    ...
    return <div key={`static-${stepIndex}`} className="border rounded-lg p-6">
        ...
        {step.primary_location && <p className="text-sm text-gray-600 mb-4">
            <strong>Lieu principal:</strong> {step.primary_location}
          </p>}
        
        <div className="space-y-4">
          {segments.map((segment: any) => <div key={segment.id} className="border-l-4 theme-border pl-4 py-2">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  {segment.provider && <p className="text-sm text-gray-600 mb-1">
                      <strong>Prestataire:</strong> {segment.provider}
                    </p>}
                  
                  {segment.reference_number && <p className="text-sm text-gray-600 mb-1">
                      <strong>Référence:</strong> {segment.reference_number}
                    </p>}
                  
                  {options.includeNotes && segment.description && <div className="mt-2 p-2 bg-gray-50 rounded text-sm">
  ...
  return <div className="space-y-8">
      {isGenerating && <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
          ...
          <div className="w-full bg-blue-200 rounded-full h-2 mb-3">
            <div className="bg-blue-600 h-2 rounded-full transition-all duration-300 ease-out" style={{
          width: `${progress}%`
        }} />
          </div>
          ...
          {currentStep >= 0 && steps.length > 0 && <div className="text-xs text-blue-500 mt-1">
      ...
      if (isGenerating || !stepContent) {
        return <div key={`loading-${stepIndex}`} className="border rounded-lg p-6">
      ...
      if (stepContent.success && stepContent.html) {
        ...
        const cleanHtml = stepContent.html.replace(/```html\n/g, '').replace(/\n```/g, '').replace(/```html/g, '').replace(/```/g, '');
        return <div key={`gpt-${stepIndex}`} className="border rounded-lg p-6 relative">
      ...
      return <div key={`fallback-${stepIndex}`}>
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-4">
              <div className="flex items-center">
                ...
                <button onClick={() => regenerateStep(stepIndex)} className="ml-auto text-yellow-600 hover:text-yellow-800" title="Réessayer la génération IA">
