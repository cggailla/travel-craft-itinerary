...
export default function TravelTimelineNew({
  ...
  const checkExistingSteps = async (tripId: string) => {
    ...
    if (exists) {
      await loadManualSteps(tripId);
    }
  };

  const loadManualSteps = async (tripId: string) => {
    try {
      const { getManualSteps } = await import('@/services/manualStepsService');
      const stepsResult = await getManualSteps(tripId);
      if (stepsResult.success && stepsResult.steps) {
        setManualSteps(stepsResult.steps);
      }
    } catch (error) {
      console.error('Error loading manual steps:', error);
    }
  };
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex space-x-2">
                {showManualGrouper && (
                  <Button 
                    onClick={() => setShowManualGrouper(false)} 
                    variant="outline"
                    size="sm"
                  >
        ...
        <CardContent>
          {showManualGrouper ? (
            ...
            <div className="space-y-6">
              {/* Affichage des étapes manuelles créées */}
              {manualSteps.map((step, stepIndex) => (
                <div key={step.id} className="relative">
                    ...
                    <div className="flex-1">
                      ...
                      {step.start_date && (
                        <div className="flex items-center space-x-2 text-sm text-muted-foreground mt-1">
                          ...
                          <span>
                            {format(new Date(step.start_date), 'EEEE d MMMM yyyy', { locale: fr })}
                            {step.end_date && step.end_date !== step.start_date && 
                              ` - ${format(new Date(step.end_date), 'EEEE d MMMM yyyy', { locale: fr })}`
                            }
                          </span>
                        </div>
                      )}
                  ...
                  <div className="ml-12 space-y-3">
                    {step.travel_step_segments
                      ?.sort((a: any, b: any) => a.position_in_step - b.position_in_step)
                      .map((tss: any) => {
                        const segment = tss.travel_segments;
