...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Detect plain text pathway (either explicit mime OR extension)
    const isPlainText = /text\/plain/i.test(mimeType) || /\.txt$/i.test(fileName);

    // When plain text: read text directly; when not: build base64 (lazy)
    ...
    if (isPlainText) {
      try {
        ...
        console.warn('Failed to read blob as text, falling back to base64 attach path', e);
        // fallback: treat as non-text
        rawPlainText = null;
      }
    }
...
console.log('Using OpenAI Responses API with ' + (isPlainText && rawPlainText ? 'inline plain-text embedding' : 'direct file attachment') + ' (two-step pipeline)');
...
// ---- BUILD MESSAGES (analysis) CONDITIONALLY ----
let analysisMessages: any[];
if (isPlainText && rawPlainText) {
  // Truncate to protect token limits (approx). Keep 45k chars max.
  ...
  const decorated = `Here is the RAW PLAIN TEXT DOCUMENT. Use it as the only source. ${clipped ? '(TRUNCATED) ' : ''}\n<<<DOCUMENT_PLAIN_TEXT_START>>>\n${truncated}\n<<<DOCUMENT_PLAIN_TEXT_END>>>`;
  analysisMessages = [
    { role: 'system', content: analysisPrompt },
    { role: 'user', content: [ { type: 'text', text: 'Analyze ONLY the document structure/layout as instructed. Do NOT parse data. Output the short analysis block.\n' + decorated } ] }
  ];
} else {
  analysisMessages = [
    { role: 'system', content: analysisPrompt },
...
// ---- BUILD MESSAGES (parsing) CONDITIONALLY ----
let parsingMessages: any[];
if (isPlainText && rawPlainText) {
  const MAX_TEXT_CHARS = 45000; // re-use same limit
  const truncated = rawPlainText.slice(0, MAX_TEXT_CHARS);
  const clipped = truncated.length < rawPlainText.length;
  const decorated = `RAW PLAIN TEXT DOCUMENT BELOW. ${clipped ? '(TRUNCATED) ' : ''}\n<<<DOCUMENT_PLAIN_TEXT_START>>>\n${truncated}\n<<<DOCUMENT_PLAIN_TEXT_END>>>`;
  parsingMessages = [
    { role: 'system', content: parsingPromptWithDirectives },
    { role: 'user', content: [ { type: 'text', text: 'Analyze this document and extract travel segments according to the instructions. Use ONLY the provided raw text.\n' + decorated } ] }
  ];
} else {
  parsingMessages = [
    { role: 'system', content: parsingPromptWithDirectives },
...
if (!extractedContent || extractedContent.trim().length === 0) {
  ...
  throw new Error('OpenAI returned empty response content');
    ...
    return new Response(JSON.stringify({
      ...
      segment_ids: createdSegments.map((s: any) => s.id),
