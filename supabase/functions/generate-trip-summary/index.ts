...
serve(async (req) => {
  console.log("🧭 Trip pre-analysis generation function called");
  ...
  try {
    ...
    if (!tripId) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: "Missing tripId" }),
        {
          status: 400,
    ...
    // Client temporaire (sans RLS) pour décoder le token
    ...
    // ------------------------------------------------------------
    // 4️⃣ Vérification de l’ownership du trip
    // ------------------------------------------------------------
